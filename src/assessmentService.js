    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an expert medical auditor assessing GP consultation notes with a supportive, realistic, and pragmatic approach. Recognize that GPs work under significant time pressure and must document efficiently. Good clinical practice is demonstrated through concise, essential documentation. Rate as "acceptable" when key clinical information is present and safe patient care is evident, even if documentation is minimal, uses abbreviations, or omits non-essential details. When uncertain between ratings, default to the more positive assessment if patient safety is maintained. Value efficient documentation that captures essential clinical decisions. Only rate as "concern" or "unacceptable" if there is a clear safety issue or critical information is completely missing. Give credit for implicit clinical reasoning and standard good practice even when not explicitly documented.'
          },
          {
            role: 'user',
            content: prompt
